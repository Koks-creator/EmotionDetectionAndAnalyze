...
class DatasetCleaner:
    ...
    def clean_data(self, dataset_path: str, file_base_name: str) -> None:
        ...
        subfolder_path = next(Path(filename).iterdir())
        all_files = glob.glob(f"{subfolder_path}/*.*")
        start = perf_counter()
...
if __name__ == '__main__':
    ...
    zip_files = [rf"people emotions face - Szukaj w Google - 28-06-2026 23-08-59.zip"]
    for index, zip_file in enumerate(zip_files):
        category = zip_file.split()[0]
        dc.clean_data(dataset_path=f"{FOLDER}/{zip_file}", file_base_name=f"{category}_{index}")
